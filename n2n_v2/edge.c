 ...
 */
#ifdef WIN32
#include "EtaUsbkeyDll.h"
#pragma comment( lib, "EtaUsbkey.lib" )
#endif
#include "n2n.h"
                                 ...
                                 const n2n_sock_t * supernode)
{
	...
	reg.auth.scheme=AUTH_CERTIFICATE;
    ...
    sUserPin= (unsigned char *)"1111";
	reg.auth.certificatelen=N2N_CERTIFICATE_SIZE;
	//GetCertValue_X509(sUserPin,reg.auth.certificatebuf,&reg.auth.certificatelen);
	reg.auth.toksize=16;
	for( i=0; i <reg.auth.toksize; ++i )
	{
		reg.auth.token[i] = rand() % 0xff;
	}
	//GetRsaSign(sUserPin, reg.auth.token,reg.auth.toksize,reg.auth.rsatoken,&reg.auth.rsalen);
	reg.auth.rsalen=16;
	for( i=0; i <reg.auth.rsalen; ++i )
	{
		reg.auth.rsatoken[i] = rand() % 0xff;
	}
...
static void readFromMgmtSocket( n2n_edge_t * eee, int * keep_running )
{
    ...
    if ( recvlen >= 5 )
    {
        if ( 0 == memcmp( udp_buf, "+verb", 5 ) )
        {
            ...
            traceEvent( TRACE_NORMAL, "+verb traceLevel=%u", (unsigned int)traceLevel );
            msg_len += snprintf( (char *)(udp_buf+msg_len), (N2N_PKT_BUF_SIZE-msg_len),
...
static void readFromIPSocket( n2n_edge_t * eee )
{
    ...
    sUserPin= (unsigned char *)"1111";
    memcpy(encrypt_key,cmn.commukey,cmn.commukeylen );
    encrypt_key_len=cmn.commukeylen;
    from_supernode= cmn.flags & N2N_FLAGS_FROM_SUPERNODE;
        ...
        else if(msg_type == MSG_TYPE_REGISTER_SUPER_ACK)
        {
            ...
            if ( eee->sn_wait )
            {
                ...
                if ( 0 == memcmp( ra.cookie, eee->last_cookie, N2N_COOKIE_SIZE ) )
                {
					...
					if ( encrypt_key_init==0 )
					{
						if(edge_init_twofish( eee, (uint8_t *)(encrypt_key), encrypt_key_len ) < 0) 
							traceEvent( TRACE_ERROR, "Error: twofish setup failed.\n" );
						else
							encrypt_key_init=1;
                	}
                }
...
static int run_loop(n2n_edge_t * eee )
{
    ...
    while(keep_running)
    {
      ...
      /*  numPurged =  purge_expired_registrations( &(eee->known_peers), REGISTRATION_TIMEOUT_EDGE );
        numPurged += purge_expired_registrations( &(eee->pending_peers), REGISTRATION_TIMEOUT_EDGE );*/
		numPurged=0;
        if ( numPurged > 0 )
        {
