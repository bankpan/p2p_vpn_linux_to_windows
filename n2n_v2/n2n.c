...
/**Find the peer entry in list with community_name equal to community.
*@return NULL if not found; otherwise pointer to peer entry.
*/
struct peer_info * find_peer_by_community( struct peer_info * list, const n2n_community_t community )
{
	while(list != NULL)
	{
		if( 0 == memcmp(community, list->community_name, 16) )
		{
			return list;
		}
		list = list->next;
...
size_t purge_expired_registrations( struct peer_info ** peer_list, int time_out ) {
  ...
  num_reg = purge_peer_list( peer_list, now - time_out );
