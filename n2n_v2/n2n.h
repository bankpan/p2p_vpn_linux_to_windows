...
#define PURGE_REGISTRATION_FREQUENCY   6 //60
#define REGISTRATION_TIMEOUT_EDGE      (PURGE_REGISTRATION_FREQUENCY * 2)
#define REGISTRATION_TIMEOUT_SUPER     (REGISTER_SUPER_INTERVAL_DFL * 2)
...
struct peer_info {
	...
	n2n_commukey_t		community_key;
	uint8_t             commukey_length;
    n2n_mac_t           mac_addr;
    n2n_sock_t          sock;
    time_t              last_seen;
};
...
size_t purge_expired_registrations( struct peer_info ** peer_list, int time_out );
struct peer_info * find_peer_by_community( struct peer_info * list, const n2n_community_t community );
int getRnd( unsigned char *s );
