...
#define N2N_COMMUKEY_SIZE               128      /* the total of the commukey length */
#define N2N_MAC_SIZE                    6
#define N2N_COOKIE_SIZE                 4
#define N2N_CERTIFICATE_SIZE			4096
#define N2N_PKT_BUF_SIZE                (2048 + N2N_CERTIFICATE_SIZE)    /* modify to large *//* origin data 2048 */
#define N2N_SOCKBUF_SIZE                64      /* string representation of INET or INET6 sockets */
#define AUTH_NONE                       0
#define AUTH_CERTIFICATE				1		/* type of cert */

typedef uint8_t n2n_community_t[N2N_COMMUNITY_SIZE];
typedef uint8_t n2n_commukey_t[N2N_COMMUKEY_SIZE];   /* rand key */
...
struct n2n_auth
{
	...
	uint32_t	rsalen; 			            /* New add */
	uint8_t     rsatoken[N2N_COMMUKEY_SIZE];						
	uint32_t	certificatelen; 				
	uint8_t     certificatebuf[N2N_CERTIFICATE_SIZE]; 				
};
...
struct n2n_common
{
	...
	uint32_t			commukeylen;
	uint8_t 			commukey[N2N_COMMUKEY_SIZE];				
};
