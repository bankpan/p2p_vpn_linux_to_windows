...
#define N2N_SN_PKTBUF_SIZE   N2N_PKT_BUF_SIZE      /* modify to large, origin data is 2048 */
...
{
    return REGISTER_SUPER_INTERVAL_DFL;
}
                        ...
                        time_t now)
{
    ...
    if ( NULL == scan )
    {
		...
		s = find_peer_by_community( sss->edges, community);           /* find the same community to add the key */
		if ( NULL == s )
		{
			scan->commukey_length = 16;
			getRnd( scan->community_key );
		}
		else
		{
			memcpy(scan->community_key, s->community_key, sizeof(n2n_commukey_t) );
		}
                        ...
                        time_t now)
{
    ...
    else if ( msg_type == MSG_TYPE_REGISTER_SUPER )
    {
		...
		scan = find_peer_by_mac( sss->edges, ack.edgeMac );
		if( NULL != scan )
		{
			cmn2.commukeylen = scan->commukey_length;
			memcpy( cmn2.commukey, scan->community_key, sizeof(n2n_commukey_t) );
		}
...
static int run_loop( n2n_sn_t * sss )
{
    ...
    while(keep_running) 
    {
        ...
        purge_expired_registrations( &(sss->edges), REGISTRATION_TIMEOUT_SUPER );
