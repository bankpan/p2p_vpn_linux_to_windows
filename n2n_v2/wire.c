                   ...
                   const n2n_common_t * common )
{
	...
	encode_uint32( base, idx, common->commukeylen );
	if(common->commukeylen > 0)
		encode_buf( base, idx, common->commukey,  common->commukeylen );
                   ...
                   size_t * idx )
{
	...
	decode_uint32( &(out->commukeylen), base, rem, idx);
	if(out->commukeylen > 0 )
		decode_buf( out->commukey, out->commukeylen, base, rem, idx );
                           ...
                           const n2n_REGISTER_SUPER_t * reg )
{
	...
	if(reg->auth.scheme==AUTH_CERTIFICATE)
	{
		retval += encode_uint16( base, idx, reg->auth.scheme);

		retval += encode_uint16( base, idx, reg->auth.toksize ); /* NULL auth scheme */
		retval += encode_buf( base, idx, reg->auth.token,  reg->auth.toksize );

		retval += encode_uint32( base, idx, reg->auth.rsalen); /* No auth data */
		retval += encode_buf( base, idx, reg->auth.rsatoken,reg->auth.rsalen );

		retval += encode_uint32( base, idx, reg->auth.certificatelen);
		retval += encode_buf( base, idx, reg->auth.certificatebuf, reg->auth.certificatelen);
	}
	else
	{
                           ...
                           size_t * idx )
{
	...
	if(reg->auth.scheme==AUTH_CERTIFICATE)
	{
		...
		retval += decode_uint32( &(reg->auth.rsalen), base, rem, idx );
		retval += decode_buf( reg->auth.rsatoken, reg->auth.rsalen, base, rem, idx );

		retval += decode_uint32( &(reg->auth.certificatelen), base, rem, idx );
		retval += decode_buf( reg->auth.certificatebuf, reg->auth.certificatelen, base, rem, idx );
	}
	else
	{
		retval += decode_uint16( &(reg->auth.toksize), base, rem, idx );
